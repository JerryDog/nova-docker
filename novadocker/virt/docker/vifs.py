...
class DockerGenericVIFDriver(object):
    ...
    def unplug(self, instance, vif):
        ...
        LOG.info('vif_type=%(vif_type)s instance=%(instance)s '
                  'vif=%(vif)s',
                  {'vif_type': vif_type, 'instance': instance,
                   'vif': vif})
    ...
    def cleanup_dir(self,datadir=None):
        try:
            utils.execute('rm', '-rf', datadir, run_as_root=True)
        except processutils.ProcessExecutionError:
            LOG.error(_('Failed to del local datadir'))
